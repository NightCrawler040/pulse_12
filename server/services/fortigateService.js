 ...
 */
export const banIpAddress = async (settings, ip) => {
  ...
  return await triggerFortigateWebhook(settings.banUrl, settings.apiToken, ip, settings.addressGroup);
};

/**
 * Снятие блокировки с IP-адреса
 */
export const unbanIpAddress = async (settings, ip) => {
  ...
  return await triggerFortigateWebhook(settings.unbanUrl, settings.apiToken, ip, settings.addressGroup);
};
