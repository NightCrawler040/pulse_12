  ...
  const clearAllNotifications = () => {
    ...
    <TaskContext.Provider value={{
      ...
      addWorkspace: async (ws) => {
        try {
          const data = await apiService.post<Workspace>('/api/workspaces', ws);
          if (data) { setWorkspaces(prev => [...prev, data]); }
        } catch (e) { console.error(e); }
      },
      updateWorkspace: async (id, updates) => {
        try {
          await apiService.put(`/api/workspaces/${id}`, updates);
          setWorkspaces(prev => prev.map(w => w.id === id ? { ...w, ...updates } : w));
        } catch (e) { console.error(e); }
      },
      deleteWorkspace: async (id) => {
        try {
          await apiService.delete(`/api/workspaces/${id}`);
          setWorkspaces(prev => prev.filter(w => w.id !== id));
        } catch (e) { console.error(e); }
      },
      addApiKey: async (name, source, workspaceId) => {
        try {
          const res = await apiService.createApiKey(name, source, workspaceId);
          if (res) {
