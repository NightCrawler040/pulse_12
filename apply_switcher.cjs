...
c = c.replace(/import \{ useAuth \} from '\.\.\/\.\.\/context\/AuthContext';/, importReplacement);

const destructReplacement = `const { viewMode, setViewMode, users, findings, workspaces, activeWorkspaceId, setActiveWorkspaceId } = useTaskContext();`;
c = c.replace(/const \{ viewMode, setViewMode, users, findings \} = useTaskContext\(\);/, destructReplacement);
